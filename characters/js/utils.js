    ...
    for (var type in window.drops) {
        for (var island=0;island<window.drops[type].length;++island) {
            ...
            for (var stage in window.drops[type][island]) {
                if (stage == 'thumb' || stage == 'name' || stage == 'shortName' || stage == 'day' || stage == 'global' || stage == 'condition' || stage == 'completion' || stage == 'challenge' || stage == 'challengeData' || stage == 'showManual' || stage == 'gamewith'|| stage == 'slefty' || stage == 'nakama' || stage == 'dropID' || stage == 'boosters' || stage == 'gimmicks') continue;
